...
(function() {
  var ALPHA_CHARS, ALPHA_NUMERIC_CHARS, ATTR_DELIM, ATTR_ENTITY_DOUBLE_DELIM, ATTR_ENTITY_NO_DELIM, ATTR_ENTITY_SINGLE_DELIM, ATTR_NAME, ATTR_NAME_MUST_GET_VALUE, ATTR_OR_TAG_END, ATTR_VALUE_DOUBLE_DELIM, ATTR_VALUE_NO_DELIM, ATTR_VALUE_SINGLE_DELIM, CHAR_OR_ENTITY_OR_TAG, CLOSING_TAG, ENTITY, ENTITY_CHARS, OPENING_TAG, OPENNING_OR_CLOSING_TAG, TAG_NAME_CLOSING, TAG_NAME_MUST_CLOSE, TAG_NAME_OPENING, TAG_OPENING_SELF_CLOSING, _Parser,
    __slice = [].slice,
  ...
  HTMLString.String = (function() {
    function String(html) {
      if (html) {
        this.characters = new _Parser().parse(html).characters;
      } else {
        this.characters = [];
      }
    }
    ...
    String.prototype.concat = function() {
      ...
      for (_j = 0, _len = strings.length; _j < _len; _j++) {
        ...
        if (typeof string === 'string') {
          tail = new HTMLString.String(string);
        }
    ...
    String.prototype.html = function() {
      var c, closingTag, closingTags, html, openHeads, openTag, openTags, tag, _i, _j, _k, _l, _len, _len1, _len2, _len3, _len4, _m, _ref, _ref1, _ref2, _ref3;
      html = '';
      openTags = [];
      openHeads = [];
      closingTags = [];
    ...
    String.prototype.insert = function(index, substring, inheritFormat) {
      ...
      middle = substring;
      if (typeof substring === 'string') {
        middle = new HTMLString.String(substring);
      }
    ...
    String.prototype.optimize = function() {
      var c, closingTag, closingTags, lastC, openHeads, openTag, openTags, runLength, runLengthSort, runLengths, run_length, t, tag, _i, _j, _k, _l, _len, _len1, _len2, _len3, _len4, _len5, _len6, _m, _n, _o, _ref, _ref1, _ref2, _ref3, _ref4, _ref5, _results;
      openTags = [];
      openHeads = [];
      lastC = null;
      ...
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        ...
        for (_m = 0, _len4 = openTags.length; _m < _len4; _m++) {
          tag = openTags[_m];
          if (!lastC) {
            c._runLengthMap[tag.head()] = [tag, 1];
            continue;
          }
          if (!c._runLengthMap[tag.head()]) {
            c._runLengthMap[tag.head()] = [tag, 0];
          }
          run_length = 0;
          if (lastC._runLengthMap[tag.head()]) {
            run_length = lastC._runLengthMap[tag.head()][1];
          }
          c._runLengthMap[tag.head()][1] = run_length + 1;
        }
        lastC = c;
      }
  ...
  _Parser = (function() {
    function _Parser() {
      ...
      this.fsm.addTransitionAny(CHAR_OR_ENTITY_OR_TAG, null, function(char) {
        return this._pushChar(char);
      });
      this.fsm.addTransition('<', CHAR_OR_ENTITY_OR_TAG, OPENNING_OR_CLOSING_TAG);
      this.fsm.addTransition('&', CHAR_OR_ENTITY_OR_TAG, ENTITY);
      this.fsm.addTransitions(ENTITY_CHARS, ENTITY, null, function(char) {
        return this.entity += char;
      });
      ...
      this.fsm.addTransitions(ALPHA_NUMERIC_CHARS, TAG_NAME_OPENING, null, function(char) {
      ...
      this.fsm.addTransitions(ALPHA_NUMERIC_CHARS, TAG_NAME_CLOSING, null, function(char) {
        return this.tagName += char;
      });
      ...
      this.fsm.addTransitions(ALPHA_NUMERIC_CHARS, ATTR_NAME, null, function(char) {
      ...
      this.fsm.addTransition('=', ATTR_NAME_MUST_GET_VALUE, ATTR_DELIM);
      this.fsm.addTransitions([' ', '\n'], ATTR_DELIM);
      ...
      this.fsm.addTransitionAny(ATTR_VALUE_NO_DELIM, null, function(char) {
        return this.attributeValue += char;
      });
      ...
      this.fsm.addTransitionAny(ATTR_VALUE_SINGLE_DELIM, null, function(char) {
        return this.attributeValue += char;
      });
      ...
      this.fsm.addTransitionAny(ATTR_VALUE_DOUBLE_DELIM, null, function(char) {
        return this.attributeValue += char;
      });
      this.fsm.addTransitions(ENTITY_CHARS, ATTR_ENTITY_NO_DELIM, null, function(char) {
        return this.entity += char;
      });
      this.fsm.addTransitions(ENTITY_CHARS, ATTR_ENTITY_SINGLE_DELIM, function(char) {
        return this.entity += char;
      });
      this.fsm.addTransitions(ENTITY_CHARS, ATTR_ENTITY_DOUBLE_DELIM, null, function(char) {
        return this.entity += char;
      });
    ...
    _Parser.prototype._pushChar = function(char) {
      var character, lastCharacter;
      character = new HTMLString.Character(char, this.tags);
    ...
    _Parser.prototype.parse = function(html) {
      var char, error;
      this.reset();
  ...
  HTMLString.Tag = (function() {
    function Tag(name, attributes) {
      ...
      this._selfClosing = HTMLString.Tag.SELF_CLOSING.indexOf(this._name) > -1;
      this._head = null;
      this._attributes = {};
      for (k in attributes) {
        v = attributes[k];
    ...
    Tag.SELF_CLOSING = 'area base br hr img input link meta'.split(' ');
  ...
  HTMLString.Character = (function() {
    ...
    Character.prototype.hasTags = function() {
      var t, tag, tagHeads, tagNames, tags, _i, _len;
      tags = 1 <= arguments.length ? __slice.call(arguments, 0) : [];
      tagNames = (function() {
        var _i, _len, _ref, _results;
        _ref = this._tags;
      ...
      for (_i = 0, _len = tags.length; _i < _len; _i++) {
        ...
        if (typeof tag === 'string') {
          ...
          if (tagHeads.indexOf(tag.head()) === -1) {
            return false;
          }
        }
      }
...
(function() {
  ...
  ContentEdit.Node = (function() {
    ...
    Node.prototype.previousContent = function() {
      return this.previousWithTest(function(node) {
        return node.content !== void 0;
      });
    };
  ...
  ContentEdit.Text = (function(_super) {
    ...
    function Text(tagName, attributes, content) {
      ...
      if (content instanceof HTMLString.String) {
        ...
        this.content = new HTMLString.String(content);
      }
    }
    ...
    Text.prototype.focus = function() {
      ...
      return Text.__super__.focus.call(this);
    };
    ...
    Text.prototype._onKeyUp = function(ev) {
      ...
      this.content = new HTMLString.String(this._domElement.innerHTML);
    ...
    Text.prototype._keyReturn = function(ev) {
      ...
      this.updateInnerHTML();
      element = new this.constructor(this._tagName, this.attr('class') ? {
      ...
      if (tip.length()) {
        ...
        return selection.select(element.domElement());
      } else {
    ...
    Text.mergers = {
      'Text': function(element, target) {
        ...
        if (element.parent()) {
          return element.parent().detach(element);
        }
      }
    };
  ...
  ContentEdit.PreText = (function(_super) {
    ...
    function PreText(tagName, attributes, content) {
      var c, inTag, preContent, _i, _len;
      inTag = false;
      preContent = '';
      if (content) {
      ...
      PreText.__super__.constructor.call(this, tagName, attributes, preContent);
    }
    ...
    PreText.prototype.html = function(indent) {
      ...
      text = content.text();
      text = text.replace(/\u00A0/g, ' ');
      return ("" + indent + "<" + this._tagName + (this._attributesToString()) + ">") + ("" + text + "</" + this._tagName + ">");
    };

    PreText.prototype._onKeyDown = function(ev) {
      if (ev.keyCode === 32) {
        this._keySpace(ev);
      }
      return PreText.__super__._onKeyDown.call(this, ev);
    };
    ...
    PreText.prototype._keyReturn = function(ev) {
      ...
      if (selection.get()[0] === 0 && selection.isCollapsed()) {
        this.content = new HTMLString.String('<br>').concat(this.content);
      } else if (this._atEnd(selection) && selection.isCollapsed()) {
        this.content = this.content.concat(new HTMLString.String('<br>'));
      } else if (selection.get()[0] === 0 && selection.get()[1] === this.content.length()) {
        this.content = new HTMLString.String('<br>');
        cursor = 0;
      } else {
        tip = this.content.substring(0, selection.get()[0]);
        tail = this.content.substring(selection.get()[1]);
        this.content = tip.concat(new HTMLString.String('<br>'), tail);
      }
      this.updateInnerHTML();
    ...
    PreText.prototype._keySpace = function(ev) {
      var cursor, selection;
      ev.preventDefault();
      selection = ContentSelect.Range.query(this._domElement);
      this.content = this.content.insert(selection.get()[0], new HTMLString.String('&nbsp;'));
      this.updateInnerHTML();
      cursor = selection.get()[0] + 1;
      selection.set(cursor, cursor);
      return selection.select(this._domElement);
...
(function() {
  ...
  window.ContentTools = {
    ...
    RESTRICTED_ATTRIBUTES: {
      'img': ['height', 'src', 'width', 'data-max-width', 'data-min-width'],
      'iframe': ['height', 'width']
    },
  ...
  ContentTools.InspectorUI = (function(_super) {
    ...
    InspectorUI.prototype.mount = function() {
      ...
      return ContentEdit.Root.get().bind('focus', this._handleFocusChange);
    };
    ...
    InspectorUI.prototype.unmount = function() {
      ...
      return ContentEdit.Root.get().unbind('focus', this._handleFocusChange);
    };
  ...
  ContentTools.TagUI = (function(_super) {
    ...
    TagUI.prototype._onMouseDown = function(ev) {
      ...
      dialog.bind('save', (function(_this) {
        return function(attributes, styles) {
          var applied, className, classNames, cssClass, name, value, _i, _j, _len, _len1, _ref, _ref1;
          dialog.unbind('save');
          for (name in attributes) {
            value = attributes[name];
            if (name === 'class') {
  ...
  ContentTools.BaseDialogUI = (function(_super) {
    __extends(BaseDialogUI, _super);

    function BaseDialogUI() {
      BaseDialogUI.__super__.constructor.call(this);
    }
  ...
  ContentTools.ImageDialog = (function(_super) {
    ...
    ImageDialog.prototype.addCropmarks = function() {
      ...
      this._cropmarks = new ContentTools.Cropmarks(this._imageSize);
  ...
  ContentTools.Cropmarks = (function(_super) {
    __extends(Cropmarks, _super);

    function Cropmarks(imageSize) {
      Cropmarks.__super__.constructor.call(this);
      this._bounds = null;
      this._dragging = null;
    ...
    Cropmarks.prototype.mount = function(domParent, before) {
      if (before == null) {
        before = null;
      }
      ...
      Cropmarks.__super__.mount.call(this, domParent, before);
      return this._fit(domParent);
    };

    Cropmarks.prototype.region = function() {
    ...
    Cropmarks.prototype.unmount = function() {
      Cropmarks.__super__.unmount.call(this);
      this._domClipper = null;
      this._domHandles = null;
      return this._domRulers = null;
    };

    Cropmarks.prototype._addDOMEventListeners = function() {
      Cropmarks.__super__._addDOMEventListeners.call(this);
    ...
    Cropmarks.prototype._drag = function(top, left) {
    ...
    Cropmarks.prototype._fit = function(domParent) {
    ...
    Cropmarks.prototype._startDrag = function(handleIndex, top, left) {
      var domHandle;
    ...
    Cropmarks.prototype._stopDrag = function() {
  ...
  ContentTools.PropertiesDialog = (function(_super) {
    ...
    PropertiesDialog.prototype.mount = function() {
      ...
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        style = _ref[_i];
        styleUI = new ContentTools.StyleUI(style, this.element.hasCSSClass(style.cssClass()));
    ...
    PropertiesDialog.prototype.save = function() {
      return this.trigger('save', this.changedAttributes(), this.changedStyles());
    };

    PropertiesDialog.prototype._addAttributeUI = function(name, value) {
      var attributeUI, dialog;
      dialog = this;
      attributeUI = new ContentTools.AttributeUI(name, value);
    ...
    PropertiesDialog.prototype._addDOMEventListeners = function() {
      ...
      this._domStylesTab.addEventListener('mousedown', (function(_this) {
        return function() {
          ContentEdit.removeCSSClass(_this._domElement, 'ct-properties-dialog--attributes');
          ContentEdit.removeCSSClass(_this._domAttributesTab, 'ct-control--active');
          ContentEdit.addCSSClass(_this._domElement, 'ct-properties-dialog--styles');
          ContentEdit.addCSSClass(_this._domStylesTab, 'ct-control--active');
          return window.localStorage.setItem('ct-properties-dialog-tab', 'styles');
        };
      })(this));
      this._domAttributesTab.addEventListener('mousedown', (function(_this) {
        return function() {
          ContentEdit.removeCSSClass(_this._domElement, 'ct-properties-dialog--styles');
          ContentEdit.removeCSSClass(_this._domStylesTab, 'ct-control--active');
          ContentEdit.addCSSClass(_this._domElement, 'ct-properties-dialog--attributes');
          ContentEdit.addCSSClass(_this._domAttributesTab, 'ct-control--active');
          return window.localStorage.setItem('ct-properties-dialog-tab', 'attributes');
        };
      })(this));
  ...
  ContentTools.AttributeUI = (function(_super) {
  ...
  _EditorApp = (function(_super) {
    ...
    _EditorApp.prototype.init = function(query, namingProp) {
      ...
      return ContentEdit.Root.get().bind('paste', (function(_this) {
        return function(element, ev) {
          var content, cursor, encodeHTML, i, insertAt, insertNode, line, lines, p, pEnd, selection, tail, tip, _i, _len;
          content = ev.clipboardData.getData('text/plain');
          lines = content.split('\n');
          ...
          if (lines.length > 1 || !element.content) {
            ...
            insertAt = insertNode.parent().children.indexOf(insertNode) + 1;
            for (i = _i = 0, _len = lines.length; _i < _len; i = ++_i) {
              line = lines[i];
              p = new ContentEdit.Text('p', {}, encodeHTML(line));
              insertNode.parent().attach(p, insertAt + i);
            }
            pEnd = p.content.length();
            p.focus();
            return p.selection(new ContentSelect.Range(pEnd, pEnd));
          } else {
            selection = element.selection();
            cursor = selection.get()[0] + content.length;
            tip = element.content.substring(0, selection.get()[0]);
            tail = element.content.substring(selection.get()[1]);
            element.content = tip.concat(new HTMLString.String(encodeHTML(content)), tail);
    ...
    _EditorApp.prototype.save = function() {
      var modifiedRegions, name, region, _ref;
      modifiedRegions = {};
      _ref = this._regions;
      for (name in _ref) {
        region = _ref[name];
        modifiedRegions[name] = region.html();
        region.domElement().innerHTML = modifiedRegions[name];
      }
      if (ContentEdit.Root.get().lastModified()) {
        return this.trigger('save', modifiedRegions);
      }
    };
  ...
  ContentTools.EditorApp = (function() {
    ...
    EditorApp.get = function() {
      return instance != null ? instance : instance = new _EditorApp();
    };
  ...
  ContentTools.StylePalette = (function() {
    ...
    StylePalette.add = function(styles) {
      this._styles = this._styles.concat(styles);
      return this._styles.sort(function(a, b) {
        if (a.name > b.name) {
          return 1;
        }
        if (a.name < b.name) {
  ...
  ContentTools.Tool = (function() {
    ...
    Tool.apply = function(element, selection) {
  ...
  ContentTools.Tools.Paragraph = (function(_super) {
    ...
    Paragraph.apply = function(element, selection, callback) {
      var paragraph, region;
      if (ContentTools.Tools.Heading.canApply(element)) {
  ...
  ContentTools.Tools.Preformatted = (function(_super) {
    ...
    Preformatted.apply = function(element, selection, callback) {
      var insertAt, parent, preText;
      preText = new ContentEdit.PreText('pre', {}, element.content.text());
  ...
  ContentTools.Tools.AlignLeft = (function(_super) {
    ...
    AlignLeft.apply = function(element, selection, callback) {
      ...
      _ref = ['text-center', 'text-left', 'text-right'];
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        className = _ref[_i];
  ...
  ContentTools.Tools.LineBreak = (function(_super) {
    ...
    LineBreak.apply = function(element, selection, callback) {
      ...
      tip = element.content.substring(0, selection.get()[0]);
      tail = element.content.substring(selection.get()[1]);
      element.content = tip.concat(new HTMLString.String('<br>'), tail);
      element.updateInnerHTML();
      element.taint();
      selection.set(cursor, cursor);
